...
class SetCriterion(nn.Module):
    ...
    def soft_label(self, outputs, targets):
        ...
        # print(outputs['aux_outputs'][0]['pred_boxes'])
        # exit()
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, epoch, log=True):
        ...
        if targets[0]['pseudo_label'] is not None:
            target_classes_onehot_teacher = torch.stack([t['pseudo_label'] for t in targets])

            loss_ce_teacher = sigmoid_focal_loss(src_logits, target_classes_onehot_teacher, num_boxes,
                                                 alpha=self.focal_alpha, gamma=self.focal_gamma) * src_logits.shape[1]
            loss_ce = 0.5 * loss_ce + 0.5 * loss_ce_teacher
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, use_aux=-1):
        ...
        return results, results_oriorder
